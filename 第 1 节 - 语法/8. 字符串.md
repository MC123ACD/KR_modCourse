...
Find and Transform 是一个强大的 VS Code 插件，它扩展了内置的查找替换功能，让你能够：
- 使用 JavaScript 脚本进行复杂的文本处理
- 在替换中使用正则表达式捕获组
- 执行数学计算和字符串操作
- 应用条件逻辑和循环

完整介绍见[[其他/Find and Transform 说明翻译]]

#### 1. 配置键盘快捷键

按 `Ctrl+Shift+P` 打开命令面板，输入 `Preferences: Open Keyboard Shortcuts (JSON)` 打开键盘快捷键配置文件。
