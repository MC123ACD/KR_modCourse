...
### 1. 折叠代码
代码块可以在左侧折叠。
![[Pasted image 20251227184445.png|400]]

### 2. 折叠标记
如果想要折叠一个区域（非代码块内）的代码可以使用 `#region` 折叠标记。
...
print("这里正常")
```
![[Pasted image 20251227190005.png|249]]
成功出现折叠箭头

### 3. 大纲
大纲可以快速查看代码结构，表结构等
![[Pasted image 20251228135103.png|875]]

### 4. 折叠标记大纲
由于大纲无法识别折叠标记，可使用 Region Viewer 插件可以为折叠标记构建大纲。

使用前需要调整设置显示 Lua 折叠标记的大纲。
...
完成
![[Pasted image 20251228135904.png|725]]
### 5. 中文翻译
使用 Code Translate 插件可以翻译鼠标光标悬浮的单词
![[Pasted image 20251228140010.png|550]]
### 6. 多光标
我们经常需要修改以下格式的代码，我们想输出 hello 一个一个删除过于麻烦，我们可以使用 `alt + 鼠标左键` 创建多个光标。
```lua
print("hello lua1")
print("hello lua2")
print("hello lua3")
...
如上我们发现一个一个点增加光标还是有点麻烦，可以使用 `shift + alt + 鼠标点击末尾` 批量创建多个光标。
...
4. **VScode 的使用**：
	- **折叠代码块**：在代码块左侧折叠
	- **折叠代码**：使用折叠标记 `--#region ... --#endregion`
	- **大纲**：使用大纲查看代码结构
	- **折叠标记大纲**：使用 Region Viewer 插件
	- **中文翻译**：使用 Code Translate 插件
	- **多光标**：使用 `alt + 鼠标左键` 创建多光标，`shift + alt + 鼠标左键末尾` 批量创建光标
