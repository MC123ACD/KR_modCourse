...
-- 敌人 1 的协程函数
local function enemy1_coroutine()
	while true do
		local event, value = coroutine.yield() -- 等待外部事件

		if event == "damage" then -- 事件为伤害指令
			hp1 = hp1 - value
			print("敌人 1：受到伤害 " .. value)
			if hp1 <= 0 then
				print("敌人 1：死亡")
				alive1 = false
				break -- 敌人死亡，结束协程
			end
		elseif event == "move" then -- 事件为移动指令
			print("敌人 1：移动一步")
		end
	end
end
...
-- 创建协程
local co1 = coroutine.create(enemy1_coroutine)
local co2 = coroutine.create(enemy2_coroutine)

-- 主循环：只要还有敌人存活就继续
while alive1 or alive2 do
	if alive1 then
		coroutine.resume(co1, "damage", 50)
	end
	if alive2 then
		coroutine.resume(co2, "damage", 40)
	end

	if alive1 then
		coroutine.resume(co1, "move")
	end
	if alive2 then
		coroutine.resume(co2, "move")
	end
end
