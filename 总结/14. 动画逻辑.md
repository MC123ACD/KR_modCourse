...
# 详细
## 一、插入逻辑
1. **插入实体时调用 `system.render:on_insert` 函数初始化 `frames`**
	1. 遍历实体的所有精灵

	2. 初始化各种参数，位置、透明度、旋转等，同时增加到实体 `render.frames` 与 `store.render_frames` 内

	3. 初始化血条的黑色条 `fk`、背景条 `fb`、前景条 `ff` 增加到实体 `health_bar.frames` 与 `store.render_frames` 内
...
1. **遍历所有具有 `render` 键的实体的 `render.sprites` 精灵**
	1. 若精灵的 `animated` 键为真，调用 `animation_db:fn` 通过前缀与精灵名称搜索动画
	...
	2. 若精灵 `animation` 键为真则直接调用 `animation_db:fni` 跳过通过精灵名称搜索动画

	3. 若精灵 `exo` 键为真则调用 `exoskeleton:f` 函数加载骨骼动画并存入精灵 `exo` 键
...
## 三、补间动画
1. **实体插入时调用 `system.tween:on_insert` 函数读取实体 `tween.props` 属性键**

2. **调用 `system.tween:on_update` 函数更新补间动画，根据关键帧进行插值计算**
