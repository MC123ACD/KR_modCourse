...
### [一] 创建模板
```lua
E:register_t(模板: str, 底模板?: str) -> 模板引用: table
```
- **作用**：使模板深拷贝底模板生成新模板，同时给模板增加 `template_name` 键，键值为模板名称

**示例**：  
```lua
tt = E:register_t("hero_gerald", "hero")
tt.health.hp = 666	-- 爵士血量改为 666
```

### [二] 添加组件
```lua
E:add_comps(模板: str, 组件: str, ...) -> nil
```
  - **作用**：向模板增加多个深拷贝的 `components` 中的组件
...
### [三] 索引模板
```lua
E:get_template(模板: str) -> 模板引用: table
```
- **作用**：返回指定模板引用
...
### [四] 拷贝组件
```lua
E:clone_c(组件: str) -> 表: table
```
- **作用**：返回深拷贝的组件，用于初始化

**示例：**
```lua
tt.melee.attacks[1] = E:clone_c("aura_attack")	-- 将近战攻击 1 赋值为范围攻击表
```
...
**示例：**
```lua
T = E:filter(store.entities, "tower")	-- 此时 T 是一个表，内有所有具有 tower 键 store.entities 内的实体
```

### [六] 筛选具有指定键的模板
```lua
E:filter_templates(键: table, ...) -> 筛选出的表: table
```
- **作用**：返回一个表，表内具有包含指定键的模板

**示例：**
```lua
T = E:filter_templates("tower")	-- 此时 T 是一个表，内有所有具有 tower 键的模板
```
