实体函数是附加到实体上的函数，用于被 `main_script` 系统调用为实体添加特定的游戏逻辑和行为。

- 存储位置： `kr/game_scripts` 与 `all/scripts` 中
...
function scripts.hero_basic.insert(this, store)
	if this.melee then	-- 排序近战攻击
		this.melee.order = U.attack_order(this.melee.attacks)
	end

	if this.ranged then	-- 排序远程攻击
		this.ranged.order = U.attack_order(this.ranged.attacks)
	end

	-- 返回 true 表示可以创建实体，否则不创建实体
	return true
end
```
...
## 3.4 更新函数
随着游戏逻辑时间更新而调用的函数（通过协程），称为更新函数（**update script**）
...
function scripts.soldier_alleria_wildcat.update(this, store)
	...
	while true do
		...
		if this.unit.is_stunned then
			...
			-- brk = break 中断，sta = status 状态
			-- 近战攻击中断或有目标，跳过返回集结点与待机
...
### 3.7.3 寻找血量最高的敌人
```lua
local targets = U.find_enemies_in_range(store.entities, hero.pos, a.min_range, a.max_range, a.vis_flags, a.vis_bans)
...
-- all/utils
-- 搜索最强（血量最高）敌人
function U.find_strongest_enemy(entities, origin, min_range, max_range, flags, bans, filter_func)
