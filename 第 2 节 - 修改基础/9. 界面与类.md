...
## 9.1 游戏界面
进入关卡后的所有视图，状态栏、能力栏、防御塔菜单等，统称为游戏界面（`all/game`）
...
修改 `sequels/kviews_screen_map_sequels.TowerRoomView:initialize` 函数增加防御塔选择槽位
```lua
...
...
## 9.4 类
类（**Class）** 是面向对象编程（**OOP**）的核心概念，它是一个用于创建对象的蓝图或模板。类定义了对象的属性（键）和方法（函数），通过类可以创建多个具有相同结构和行为的对象（称为实例）。
...
### 6. 核心继承链
![[Pasted image 20260106151123.png|725]]
...
| 键                | 描述              | 键               | 描述   |
| ---------------- | --------------- | --------------- | ---- |
| `pos = vec2`     | 位置，注：是相对于父视图的位置 | `scale = vec2`  | 缩放   |
...
| ![[Pasted image 20260103142735.png\|400]] | ![[Pasted image 20260103142802.png\|460]] |
| ----------------------------------------- | ----------------------------------------- |
...
## 9.7 坐标
游戏中使用多种坐标系统来定位和转换对象位置，理解这些坐标系统之间的转换关系至关重要。

### 9.7.1 各种坐标
#### 1. 游戏坐标（Game Coordinates/World Coordinates）
**定义：** 游戏世界中的绝对坐标，用于定位游戏实体、场景元素等。  
**特点：**
- 原点通常位于游戏世界中的某个参考点
- 与游戏逻辑直接相关
- 可以超出屏幕可见范围
...
```lua
-- 实体位置使用游戏坐标
entity.position = v(100, 200)  -- 游戏坐标
```

#### 2. 屏幕坐标（Screen Coordinates）
**定义：** 屏幕上的绝对像素坐标，原点位于屏幕左上角(0, 0)。  
**特点：**
- 原点固定在屏幕左上角
- x轴向右递增，y轴向下递增
- 与显示分辨率直接对应
...
-- 假设点击了屏幕中心：mouse_x = 640, mouse_y = 360 (1280x720分辨率中心)
```

#### 3. 视图坐标（View Coordinates）
**定义：** 相对于特定视图的局部坐标，考虑视图的变换（位置、缩放、旋转）。  
**特点：**
- 相对于视图的锚点（通常为左上角）
- 考虑视图层级关系
- 用于UI元素定位
...
### 9.7.2 具体视图坐标转换
#### 视图层级结构：
![[Pasted image 20260106143849.png|550]]
```text
KWindow (顶层视图)
├─位置：(0, 0)
├─锚点：左上角
...
```lua
-- 具体计算代码
for i = #parent_views, 1, -1 do
	local v = parent_views[i]
...
-- 已知参数
local screen = (15, 10)  -- 屏幕坐标
local parent_view = (10, 5) -- 父视图 KView 的位置
local window = (0, 0)       -- KWindow 位置

-- 为了简洁将锚点设为 (0, 0)（左上角），缩放设为 (1, 1)，故不参与计算
-- 根据代码算式为：
-- V 为视图坐标
local V = screen - parent_view - window
-- V = (15, 10) - (10, 5) - (0, 0)
-- V = (5, 5)
```
...
#### 1. u2g - 屏幕坐标转游戏坐标
```lua
game_gui.u2g(屏幕坐标: vec2, 是否取整对齐: bool) -> 游戏x, 游戏y
```
- 通常用于与 `KWindow:getPosition` 组合使用获取鼠标光标位置

##### 示例
```lua
-- 更新函数
function scripts.entity.update(this, store)
	while true do
		-- 获取鼠标光标坐标（屏幕坐标）
...
#### 2. g2u - 游戏坐标转屏幕坐标
```lua
game_gui.g2u(游戏坐标: vec2) -> 屏幕x, 屏幕y
```

#### 3. screen_to_view - 屏幕坐标转视图坐标
```lua
:screen_to_view(屏幕x, 屏幕y) -> 视图x, 视图y
```
- 自动取整对齐
- 通常用于获取相对于视图的坐标，如点击回调事件中的点击位置

#### 4. view_to_screen - 视图坐标转屏幕坐标
```lua
:view_to_screen(视图x, 视图y) -> 屏幕x, 屏幕y
```
- 自动取整对齐

#### 5. view_to_view - 视图坐标转视图坐标
```lua
:view_to_view(视图x, 视图y, 另一个视图: class) -> 视图x, 视图y
```
- 自动取整对齐
- 将一个视图坐标转换为另一个视图坐标
...
##### 更规范的检测点击按钮的左侧还是右侧
```lua
-- 上面的版本若镶套一层视图将会导致坐标偏差
function item_button.on_click(this, button, vx, vy)
	local ix = this:view_to_view(vx, vy, this.parent)

	if ix < this.pos.x + this.size.x / 2 then
		config[key] = string.format("%.1f", config[key] - 0.1)
	else
		config[key] = string.format("%.1f", config[key] + 0.1)
	end
