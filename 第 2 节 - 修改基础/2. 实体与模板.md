...
* **作用**：它的唯一作用是将不同的 z 组件组合在一起，形成一个有意义的游戏对象。例如，一个“玩家”实体，可能是 `entity #1`，它关联了 `位置`、`精灵`、`生命值`、`控制器` 等组件。
...
3. **系统运作**：
    ...
    *  **渲染系统** 每帧运行。寻找所有同时拥有 `position` 和 `velocity` 的实体。找到 `entity #1`，读取其位置和纹理数据，将其绘制在屏幕正确位置。
