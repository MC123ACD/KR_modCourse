...
### 2.1.1 ECS 架构
开始之前我们需要先了解 ECS（**Entity-Component-System**）实体-组件-系统是什么

ECS 是一种**游戏编程架构模式**，核心思想是将数据与逻辑分离，强调组合优于继承。

#### 三核心部分
##### E - Entity（实体）
* 一个唯一的**标识符**（通常就是一个 ID），它不是游戏对象本身，而是一个“容器”。
* **类比**：就像一张**空白的身份证**，上面只有一个身份证号码。这张证本身没有任何信息（没有名字、年龄、职业），但它可以用来关联各种信息卡片。
* **作用**：它的唯一作用是将不同的组件组合在一起，形成一个有意义的游戏对象。例如，一个“敌人”实体， `entity #1`，关联了 `位置`、`精灵`、`生命值`、`控制器` 等组件。

##### 2. C - Component（组件）
* **纯数据**的容器。它只包含属性，**绝对不包含任何逻辑或方法**。
* **类比**：一张张**信息卡片**。比如“位置卡”只记录 `(x, y, z)` 坐标；“生命值卡”只记录 `当前血量` 和 `最大血量`；“渲染卡”只记录 `纹理ID` 和 `颜色`。
* **作用**：定义实体的**特征**。一个实体拥有哪些组件，就具备了哪些能力。想给实体添加新功能？只需挂载一个新的组件即可，无需修改复杂的类继承树。

##### 3. S - System（系统）
* **纯逻辑**的执行者。它只包含行为和函数，**不持有自己的状态**（数据来自组件）。
* **类比**：一个个**专业的处理部门**。比如“移动部门”只关心所有带有 `位置` 和 `速度` 组件的实体，并根据速度更新它们的位置；“渲染部门”只关心所有带有 `位置` 和 `精灵` 组件的实体，并把它们画到屏幕上。
* **作用**：驱动游戏的运转。系统遍历所有拥有特定组件的实体，并对这些组件的**数据进行操作**。

#### ECS 如何协同工作

假设我们要创建一个可以移动和渲染的“敌人”。

1. **创建实体**：生成一个 `entity #1`（只是一个 ID）。
2. **添加组件**：为 `entity #1` 添加：
    *  `position` (x: 0, y: 0)
    *  `velocity` (vx: 1, vy: 0)
    *  `sprite` (texture: “enemy.png”)
3. **系统运作**：
    *  **移动系统** 每帧运行。寻找所有同时拥有 `position` 和 `velocity` 的实体。找到 `entity #1`，读取其速度数据，更新其位置数据
    *  **渲染系统** 每帧运行。寻找所有拥有 `sprite` 的实体。找到 `entity #1`，读取其位置和纹理数据，将其绘制在屏幕正确位置

实体发生了什么、看起来什么样，完全由它身上挂载的组件组合决定，而行为则由系统统一驱动。

### 2.1.2 游戏中的定义

`all/entity_db` 模块简称为 E

##### 1. 实体模板（templates）
- **定义**： `E.entities` 表中包含的子表，称为实体模板
- **作用**：使用模板创建各种实体，用于存储实体的数据
- 数据位置：`kr/game_templates` 与 `all/templates`、 `kr/data/balance`
- 模板的表无实际作用，仅用于存储数据，数据传递过程： `balance` → 模板 → 实体 → 函数
...
### 2.2.1 修改基础属性
#### 修改方法
首先使用搜索功能在 `kr/game_templates` 任意找到一个英雄模板，所有模板名见 [[1. 模板]]
...
首先查资料（见[[1. 模板]] ）知道：四技能的名称是 `double_strike`
```lua
