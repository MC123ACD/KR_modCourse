...
## 6.5 音效（sound）
音效资源存储于 `_assets/kr-desktop/sounds` （三代为 `kr3-desktop/assets/kr-desktop/sounds`）目录中。
...
#### 1. 将音效加入播放队列
```lua
S:queue(音效名称: str, 音效参数?: table)
```
- 同样会分配唯一 ID

##### 可选音效参数
```lua
1. gain: 音效大小: num|list{最小范围: num, 最大范围: num}
...
## 6.6 加载资源
为了性能游戏通常只会加载 `required_textures` 纹理与 `required_sounds` 音效中指定的资源
