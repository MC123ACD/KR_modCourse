...
**读取位置**：读取 `kr/data/levels` 中的关卡路径数据（`_paths`）
...
### 4.3.2 修改出怪
由于出怪数据的表不易于查看修改，推荐使用附件的波次生成脚本修改
...
如图所示绿色部分的单元格类型为不可移动，也就是说修改英雄集结点英雄只会在灰色部分寻路，关卡内按 `g` 显示网格

**存储位置**：`all/grid_db` 的 `grid` 表中
**读取位置**：读取 `kr/data/levels` 中的关卡网格数据（`_grid`）