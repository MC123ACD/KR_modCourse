...
一般事件监听都在 `kr/achievements_handlers` 中
通过 signal 信号库的 `signal.register` 注册与 `signal.emit` 触发信号进行事件监听

signal 信号库函数见 [[11. 库#signal 信号库]]

### 7.3.1 什么是信号？

信号是一种**发布-订阅（Publish-Subscribe）模式**的实现，用于在软件的不同部分之间进行松耦合的通信。它就像现实生活中的广播系统——广播站发出信号，所有调谐到这个频率的接收器都能收到消息。
...
##### 2. 订阅者（Subscriber）
- 负责**监听信号**（register）
- 告诉系统："我对某某事件感兴趣，发生时请通知我"
- 提供处理函数（回调函数）

### 7.3.2 函数
#### 1. signal.register - 注册信号
```lua
